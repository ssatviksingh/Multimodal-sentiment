"""
convnext_tiny_train.py — CNN baseline for multimodal benchmark
"""
...
for ep in range(5):
    ...
    if vl_acc > best_acc:
        best_acc = vl_acc
        torch.save(model.state_dict(), os.path.join(OUT_DIR, "convnext_tiny_best.pt"))

pd.DataFrame(logs, columns=["epoch","train_loss","train_acc","val_loss","val_acc"]).to_csv(
    os.path.join(OUT_DIR, "convnext_tiny_log.csv"), index=False)
print("✅ Saved: convnext_tiny_best.pt + convnext_tiny_log.csv")
