"""
efficientnet_b0_train.py — CNN baseline for multimodal benchmark
"""
...
for ep in range(5):
    ...
    if vl_acc > best_acc:
        best_acc = vl_acc
        torch.save(model.state_dict(), os.path.join(OUT_DIR, "efficientnet_b0_best.pt"))

pd.DataFrame(logs, columns=["epoch","train_loss","train_acc","val_loss","val_acc"]).to_csv(
    os.path.join(OUT_DIR, "efficientnet_b0_log.csv"), index=False)
print("✅ Saved: efficientnet_b0_best.pt + efficientnet_b0_log.csv")
